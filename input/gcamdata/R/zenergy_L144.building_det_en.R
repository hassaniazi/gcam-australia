...
module_energy_L144.building_det_en <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    if (!"Australia" %in% A44.Calprice_bld$region) {
      A44.Calprice_bld %>%
        filter(region == "NZ") %>%
    ...
    # A44.Calprice_bld has regional data. Will error while regional breakouts.
    # Stop with a warning by checking regions against GCAM_region_names
    if (!all(GCAM_region_names$region %in% unique(A44.Calprice_bld$region))) {
    ...
    L144.prices_bld <- A44.Calprice_bld %>%
      ...
      gather_years() %>%
      separate(market, c("market", "cons.groups"), sep = "_", ) %>%
    ...
    L144.prices_bld %>%
      ...
      add_precursors("energy/A44.CalPrice_bld", "energy/calibrated_techs_bld_det",
                     "L101.in_EJ_ctry_bld_Fi_Yh", "common/GCAM_region_names") ->
      L144.prices_bld
